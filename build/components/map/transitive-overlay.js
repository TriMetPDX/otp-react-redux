...
function (_MapLayer) {
  ...
  _createClass(TransitiveCanvasOverlay, [{
    ...
    value: function componentDidMount() {
      var map = this.context.map; // FIXME: hack because L is undefined for some reason

      var LEAFLET = _leaflet.default || window.L;
      LEAFLET.canvasLayer().delegate(this) // -- if we do not inherit from L.CanvasLayer  we can setup a delegate to receive events from L.CanvasLayer
      .addTo(map);
    }
  }, {
    ...
    value: function _initTransitive(canvas) {
      var map = this.context.map; // set up the transitive instance
...
_defineProperty(TransitiveCanvasOverlay, "propTypes", {
  transitiveData: _react.PropTypes.object // React Lifecycle Methods
...
var _default = (0, _reactRedux.connect)(mapStateToProps, mapDispatchToProps)(TransitiveCanvasOverlay);
